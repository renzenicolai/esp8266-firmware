/* File: ws2801.c
 * Description: Driver for ws2812 ledstips using the i2s interface (uses uart-RX pin as output!)
 * Author: Charles Lohr
 * Date: 2015
 * License: Public domain (part of code that drives ESP8266 i2s bus is ESPRESSIF MIT licensed)
 */
...
#include <esp8266.h>
  ...
  bool ICACHE_FLASH_ATTR ws2812_init( uint16_t pixels, uint8_t r, uint8_t g, uint8_t b )
  {
    ...
    amount = pixels;
    buffersize = pixels*3;
    data = (uint8_t*) os_malloc(pixels*3); //Malloc buffer (3 bytes per pixel: G, R and B) (... does not work, add one to keep the thing happy!)
    os_printf("WS2812_SIZE:%d\r\n", pixels);
    if (!data) return false; //Return if malloc failed
    uint16_t pos;
  ...
  void ICACHE_FLASH_ATTR ws2812_send( bool clear )
  {
    if (!data) return;
    uint16_t place;
    ...
    //  while( !ws2812_dma_complete );
      ...
      {
        uint8_t b;
        b = data[pl++]; uint16_t c1a = bitpatterns[(b&0x0f)]; uint16_t c1b = bitpatterns[(b>>4)];
        b = data[pl++]; uint16_t c2a = bitpatterns[(b&0x0f)]; uint16_t c2b = bitpatterns[(b>>4)];
        b = data[pl++]; uint16_t c3a = bitpatterns[(b&0x0f)]; uint16_t c3b = bitpatterns[(b>>4)];
        b = data[pl++]; uint16_t c4a = bitpatterns[(b&0x0f)]; uint16_t c4b = bitpatterns[(b>>4)];
  ...
  void ICACHE_FLASH_ATTR ws2812_push(uint8_t r, uint8_t g, uint8_t b)
  {
    ...
    for (pixel = amount-1; pixel>0; pixel--) {
      data[pixel*3] = data[(pixel-1)*3];
      data[pixel*3+1] = data[(pixel-1)*3+1];
      data[pixel*3+2] = data[(pixel-1)*3+2];
    }
    data[0] = g; //Set first pixel to value
    data[1] = r;
    data[2] = b;
  }
  ...
  {
    if (!data) return; //Ignore when data buffer has not been initialized
