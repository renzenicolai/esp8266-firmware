...
#define ARTNET_FIRMWARE_VERSION 		0x0310	// DMX-Hub firmware version.
...
void ICACHE_FLASH_ATTR artnet_sendPollReply (void)
{
  ...
  if (espconn_send(&artnetconn,(uint8_t*)&msg,sizeof(struct artnet_pollreply)) == 0) {
    os_printf("Sent reply!\r\n");
  } else {
    os_printf("Error while sending reply!\r\n");
  }
  //reply_transmit--;
}
...
void ICACHE_FLASH_ATTR processIpProgPacket (struct espconn *conn, struct artnet_ipprog *ipprog, unsigned short packetlen)  {
	...
	if ((ipprog->command & 128) == 128) {	// enable programming
		...
		if ((ipprog->command & 4) == 4) {
			struct ip_info ipconfig;                        
			os_printf("Artnet IP-prog packet received: " IPSTR "\r\n", IP2STR(ipprog->progIp));
...
static void ICACHE_FLASH_ATTR artnet_recv_opoutput(unsigned char *data, unsigned short packetlen)
{
	...
	if (dmx->universe == ((artnet_subNet << 4) | artnet_outputUniverse1)) 
	{
		...
		if(artnet_dmxChannels > MAX_CHANNELS) artnet_dmxChannels = MAX_CHANNELS;
                ...
                uint8_t* dmx_data = dmxGetBuffer();

                uint16_t tmp;
		for(tmp = 0;tmp<513;tmp++)
		{
...
static void ICACHE_FLASH_ATTR artnet_get(void *arg, char *data, unsigned short length) {
	...
	os_printf("Received packet on Artnet port!!!\r\n");
...
//	reply_transmit = 0;
	...
	if (espconn_create(&artnetconn) == 0) {
          os_printf("ARTNET STARTED\r\n");
        } else {
          os_printf("ERROR WHILE CREATING ARTNET SOCKET\r\n");
        }
}
