...
static uint32 get_gpio16() {
  //Set GPIO0 to input
  CLEAR_PERI_REG_MASK(PERIPHS_IO_MUX_GPIO0_U, (PERIPHS_IO_MUX_FUNC<<PERIPHS_IO_MUX_FUNC_S));
  SET_PERI_REG_MASK(PERIPHS_IO_MUX_GPIO0_U, (((FUNC_GPIO0&BIT2)<<2)|(FUNC_GPIO0&0x3))<<PERIPHS_IO_MUX_FUNC_S);
  ...
  //ets_printf("Press button to enter recovery mode...\r\n");
  //ets_delay_us(2000000);
  uint32 x = GPIO_INPUT_GET(0);//(READ_PERI_REG(RTC_GPIO_IN_DATA) & 1); 
  return x;
}
...
#define APB_CLK_FREQ 52*1000000 //unit: Hz
#define UART0 0
...
uint32 NOINLINE find_image() {
  ...
  uart_div_modify(UART0, APB_CLK_FREQ / 115200 );
  ets_delay_us(1000000);
  ets_printf("rBoot v1.2.1\r\n");
...
/* #ifdef BOOT_BIG_FLASH
  ...
  if (romconf->magic != BOOT_CONFIG_MAGIC || romconf->version != BOOT_CONFIG_VERSION
    ...
    ets_printf("Writing default boot configuration...\r\n");
    ...
    romconf->gpio_rom = 1;
    romconf->roms[0] = SECTOR_SIZE*0x001; //SECTOR_SIZE * (BOOT_CONFIG_SECTOR + 1);
    romconf->roms[1] = SECTOR_SIZE*0x101; //(flashsize / 2) + (SECTOR_SIZE * (BOOT_CONFIG_SECTOR + 1));
#ifdef BOOT_CONFIG_CHKSUM
    romconf->chksum = calc_chksum((uint8*)romconf, (uint8*)&romconf->chksum);
#endif
  ...
  do {
    ...
    if (runAddr == 0) {
      if (gpio_boot) {
        ets_printf("Recovery (rom %d) is bad.\r\n", romToBoot);
        gpio_boot = FALSE;
      } else {
      ...
      /*if (gpio_boot) {
        ...
        if (romToBoot == romconf->current_rom) {
          ...
          ets_printf("Boot failed: No valid firmware available.\r\n");
          return 0;
        }
      //}
    }
