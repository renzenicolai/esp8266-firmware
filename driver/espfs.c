...
/* Functions:
 * readFlashUnaligned: Copies len bytes over from dst to src, but does it using *only* aligned 32-bit reads.
 * espFsInit: init espfs
 * espFsFlags: Returns flags of opened file.
 * espFsOpen: Open a file and return a pointer to the file desc struct.
 * espFsRead: Read len bytes from the given file into buff. Returns the actual amount of bytes read.
 * espFsClose: Close file
 */
