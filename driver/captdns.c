...
static uint16_t ICACHE_FLASH_ATTR sprite_ntohs(uint16_t *in) {
	char *p=(char*)in;
...
static char* ICACHE_FLASH_ATTR labelToStr(char *packet, char *labelPtr, int packetSz, char *res, int resMaxLen) {
	...
	do {
		if ((*labelPtr&0xC0)==0) {
			...
			endPtr=labelPtr+2;
			int offset=sprite_ntohs(((uint16_t *)labelPtr))&0x3FFF;
...
static void ICACHE_FLASH_ATTR captdnsRecv(void* arg, char *pusrdata, unsigned short length) {
	...
	os_printf("DNS packet: id 0x%X flags 0x%X rcode 0x%X qcnt %d ancnt %d nscount %d arcount %d len %d\n", 
		sprite_ntohs(&hdr->id), hdr->flags, hdr->rcode, sprite_ntohs(&hdr->qdcount), sprite_ntohs(&hdr->ancount), sprite_ntohs(&hdr->nscount), sprite_ntohs(&hdr->arcount), length);
	//Some sanity checks:
	...
	rhdr->flags|=FLAG_QR;
	for (i=0; i<sprite_ntohs(&hdr->qdcount); i++) {
		...
		if (sprite_ntohs(&qf->type)==QTYPE_A) {
			...
			*rend++=ip4_addr4(&info.ip);
			setn16(&rhdr->ancount, sprite_ntohs(&rhdr->ancount)+1);
			os_printf("Added A rec to resp. Resp len is %d\n", (rend-reply));
		} else if (sprite_ntohs(&qf->type)==QTYPE_NS) {
		...
		} else if (sprite_ntohs(&qf->type)==QTYPE_URI) {
			//Give uri to us
			rend=strToLabel(buff, rend, sizeof(reply)-(rend-reply)); //Add the label
			DnsResourceFooter *rf=(DnsResourceFooter *)rend;
			rend+=sizeof(DnsResourceFooter);
	...
	//Send the response
	espconn_send(conn, (uint8*)reply, rend-reply);
}
